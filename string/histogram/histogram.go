...
func New(
	shardsCount uint32, shardFn func(string) uint32,
) (*Histogram, error) {
	if shardFn == nil {
...
func (h *Histogram) getShardFor(key string) *histogramShard {
...
func (h *Histogram) Add(key string, amount uint64) {
...
func (h *Histogram) Get(key string) uint64 {
	shard := h.getShardFor(key)
	shard.RLock()
...
func (h *Histogram) VisitAll(fn func(string, uint64) bool) {
	for _, sh := range h.shards {
		sh.RLock()
	}
	defer func() {
