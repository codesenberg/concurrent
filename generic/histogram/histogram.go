...
type Histogram struct {
	...
	shardFn func(generic.T) uint32
}

type histogramShard struct {
	sync.RWMutex
	counters map[generic.T]uint64
}
...
func New(
	shardsCount uint32, shardFn func(generic.T) uint32,
) (*Histogram, error) {
	if shardFn == nil {
	...
	for i := range shards {
		shards[i] = &histogramShard{
			counters: make(map[generic.T]uint64),
		}
	}
	return &Histogram{
		shards:  shards,
...
func (h *Histogram) getShardFor(key generic.T) *histogramShard {
...
func (h *Histogram) Increment(key generic.T) {
	h.Add(key, 1)
}

// Add increments counter associated with the specified key by
// the amount specified.
func (h *Histogram) Add(key generic.T, amount uint64) {
...
func (h *Histogram) Get(key generic.T) uint64 {
	shard := h.getShardFor(key)
	shard.RLock()
