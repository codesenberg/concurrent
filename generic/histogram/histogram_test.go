...
func TestHistogramVisitAll(t *testing.T) {
	h := testHistogram()
	keys := testValues()
	keysMap := make(map[generic.T]struct{})
	for _, k := range keys {
		h.Increment(k)
		keysMap[k] = struct{}{}
	}
	foundKeys := make(map[generic.T]struct{})
	h.VisitAll(func(key generic.T, c uint64) bool {
		if c != 1 {
...
func benchmarkHistogramPerformance(p int, data []generic.T) func(*testing.B) {
