...
func TestHistogramVisitAll(t *testing.T) {
	h := testHistogram()
	keys := testValues()
	keysMap := make(map[float64]struct{})
	for _, k := range keys {
		h.Increment(k)
		keysMap[k] = struct{}{}
	}
	foundKeys := make(map[float64]struct{})
	h.VisitAll(func(key float64, c uint64) bool {
		if c != 1 {
...
func benchmarkHistogramPerformance(p int, data []float64) func(*testing.B) {
